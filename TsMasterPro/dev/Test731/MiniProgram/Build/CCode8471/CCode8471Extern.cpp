...
DLLEXPORT s32 __stdcall retrieve_mp_abilities(const void* AObj, const TRegTSMasterFunction AReg) {
  ...
  if (!AReg(AObj, "var", "varCAN1", "3,ezA_,MHgx", reinterpret_cast<const void*>(&varCAN1), "")) return -1;
  if (!AReg(AObj, "timer", "NewTimer1", "10", reinterpret_cast<const void*>(&NewTimer1), "")) return -1;
